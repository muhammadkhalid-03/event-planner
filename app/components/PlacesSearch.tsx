...
import { usePlacesSearch, PlacesSearchParams, DetailedPlaceResult, PLACE_TYPES } from '../hooks/usePlacesSearch';
...
export const PlacesSearch: React.FC = () => {
  ...
  const [selectedPlaceTypes, setSelectedPlaceTypes] = useState<string[]>(['restaurant', 'park', 'night_club']);
  ...
  const fetchPlaceDetails = async (placeId: string, placeType: string): Promise<DetailedPlaceResult | null> => {
    try {
  ...
  const savePlacesToFile = async (places: any[], placeTypeFilter: string) => {
    try {
      ...
      console.log(`Fetching details for ${filteredPlaces.length} ${placeTypeFilter} places...`);
      const detailedPlaces: DetailedPlaceResult[] = [];
      
      for (let i = 0; i < filteredPlaces.length; i++) {
        const place = filteredPlaces[i];
        console.log(`Fetching details for ${place.displayName} (${i + 1}/${filteredPlaces.length})`);
      ...
      setDetailedPlaces(prev => {
        ...
        const filtered = prev.filter(p => p.placeType !== placeTypeFilter || placeTypeFilter === 'all');
      ...
      if (result.success) {
        console.log(`Successfully saved ${detailedPlaces.length} ${placeTypeFilter} places with detailed information`);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <form onSubmit={handleLocationSubmit} className="mb-8 space-y-4">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium mb-2">Place Types to Search</label>
            ...
            {Object.entries(PLACE_TYPES).map(([key, value]) => (
              <label key={key} className="flex items-center space-x-2 cursor-pointer">
                ...
                <span className="text-sm">
                  {getPlaceTypeIcon(key.slice(0, -1))} {key.charAt(0).toUpperCase() + key.slice(1)}
                </span>
              </label>
            ))}
          </div>
        ...
        <div className="flex gap-4">
          ...
          >
            {isSearching ? 'Searching...' : isLoadingDetails ? 'Loading Details...' : 'Search Places'}
          </button>
        ...
        {(isSearching || isLoadingDetails) && (
          <div className="text-sm text-gray-600">
            ...
            {isLoadingDetails && "📋 Fetching detailed information for each place..."}
          </div>
        )}
      </form>
      ...
      {coordinates && (
        <div className="mb-4 p-4 bg-gray-100 rounded">
          ...
          <p><strong>Total Places Found:</strong> {places.length}</p>
          {Object.entries(groupedPlaces).map(([type, places]) => (
            <p key={type}>
              <strong>{getPlaceTypeIcon(type)} {getPlaceTypeLabel(type)}s:</strong> {places.length}
            </p>
          ))}
          {detailedPlaces.length > 0 && (
            <p><strong>Detailed Data Available:</strong> {detailedPlaces.length} places</p>
          )}
          {detailedPlaces.length > 0 && (
            <div className="mt-3">
              <Link 
          ...
          {Object.entries(groupedDetailedPlaces).map(([type, places]) => (
            <div key={type} className="space-y-4">
              <h2 className="text-2xl font-bold border-b pb-2">
                {getPlaceTypeIcon(type)} {getPlaceTypeLabel(type)}s ({places.length})
              </h2>
                ...
                {places.map((place) => (
                  <div key={place.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className="text-lg">{getPlaceTypeIcon(place.placeType)}</span>
                    </div>
