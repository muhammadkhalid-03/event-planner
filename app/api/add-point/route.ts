...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎯 Using selected place types: ${selectedPlaceTypes?.join(', ') || 'fallback types'}`);

    // Use the originally selected place types from Gemini, with fallback
    const placeTypesToUse = selectedPlaceTypes && selectedPlaceTypes.length > 0 
      ? selectedPlaceTypes 
    ...
    const existingIds = new Set<string>(currentLocations.map((loc: any) => String(loc.id)));
    let availablePlaces = placesData.filter(place => 
      !existingIds.has(String(place.id))
    );

    console.log(`🔍 Filtered out ${placesData.length - availablePlaces.length} existing locations from ${placesData.length} found places`);

    // Always try api_logs for more variety when we have limited options
    if (availablePlaces.length < 15) {
      console.log(`🔄 Only ${availablePlaces.length} alternatives found, expanding with api_logs...`);
      const additionalPlaces = await getPlacesFromApiLogs(startingLocation, radius, placeTypesToUse, existingIds);
      availablePlaces.push(...additionalPlaces);
      
      // Remove duplicates after combining
      availablePlaces = availablePlaces.filter((place, index, self) => 
        index === self.findIndex(p => p.id === place.id)
      );
      
      console.log(`📈 Expanded to ${availablePlaces.length} total alternatives for adding`);
    }
    
    // If still limited, try a broader search
    if (availablePlaces.length < 10) {
      console.log("🔍 Still limited alternatives for adding, trying broader search...");
      const broaderPlaces = await searchNearbyPlaces({
        latitude: startingLocation.location.lat,
        longitude: startingLocation.location.lng,
        radiusInMeters: (radius || 1000) * 2,
        placeTypes: ['restaurant', 'cafe', 'park', 'museum', 'tourist_attraction', 'art_gallery', 'shopping_mall'],
      });
      
      const newBroaderPlaces = broaderPlaces.filter(place => 
        !existingIds.has(String(place.id)) && 
        !availablePlaces.find(existing => existing.id === place.id)
      );
      
      availablePlaces.push(...newBroaderPlaces);
      console.log(`🌐 Added ${newBroaderPlaces.length} places from broader search for adding`);
    }

    if (availablePlaces.length === 0) {
      return NextResponse.json(
        { success: false, error: "No additional places found even after checking saved data" },
        { status: 404 }
      );
    }
    ...
    const underrepresentedTypes = placeTypesToUse.filter((type: string) => 
      (typeCounts[type] || 0) < 2 // Less than 2 of this type
    );
    ...
    if (underrepresentedTypes.length > 0) {
      ...
      if (underrepresentedPlaces.length > 0) {
        ...
        console.log(`🎯 Prioritizing underrepresented types: ${underrepresentedTypes.join(', ')}`);
      }
    }
    
    // If we still have too few options, expand the candidate pool
    if (candidatePlaces.length < 5) {
      candidatePlaces = availablePlaces.filter(place => 
        (place.rating || 0) >= 3.5 // Any decent place
      );
    }
    ...
    const sortedCandidates = candidatePlaces
      .sort((a, b) => (b.rating || 0) - (a.rating || 0));
    
    // Pick randomly from top candidates to add variety
    const topCandidates = sortedCandidates.slice(0, Math.min(8, candidatePlaces.length));
    const randomIndex = Math.floor(Math.random() * topCandidates.length);
    newLocation = topCandidates[randomIndex];
    
    console.log(`➕ Selected from ${candidatePlaces.length} candidates, picked: ${newLocation.displayName} (${newLocation.placeType})`);
...
async function getPlacesFromApiLogs(
  ...
  try {
    ...
    const relevantFiles = files.filter(file => 
      (file.startsWith('event-plan-places-') || file.startsWith('multiple-routes-places-')) && 
      file.endsWith('.json')
    ).sort((a, b) => {
    ...
    for (const file of relevantFiles.slice(0, 10)) {
      try {
        ...
        const matchingPlaces = data.places.filter((place: any) => {
          ...
          const isCommonVenue = ['restaurant', 'cafe', 'park', 'museum', 'tourist_attraction', 
                                  'art_gallery', 'shopping_mall', 'movie_theater'].includes(place.placeType);
    ...
    const uniquePlaces = additionalPlaces.filter((place, index, self) => 
      index === self.findIndex(p => p.id === place.id)
    ).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    
    console.log(`📂 Found ${uniquePlaces.length} additional places from api_logs`);
    return uniquePlaces;
