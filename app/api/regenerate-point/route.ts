...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingIds = new Set<string>((allCurrentLocations || []).map((loc: any) => String(loc.id)));
    ...
    if (availablePlaces.length < 15) {
      ...
      console.log(`📈 Expanded to ${availablePlaces.length} total alternatives`);
    }
    ...
    if (availablePlaces.length < 10) {
      ...
      const broaderPlaces = await searchNearbyPlaces({
        ...
        placeTypes: ['restaurant', 'cafe', 'park', 'museum', 'tourist_attraction', 'art_gallery', 'shopping_mall'], // Common types
      });
      ...
      console.log(`🌐 Added ${newBroaderPlaces.length} places from broader search`);
    }

    if (availablePlaces.length === 0) {
      return NextResponse.json(
        { success: false, error: "No alternative places found even after checking saved data" },
        { status: 404 }
      );
    }
    ...
    if (candidatePlaces.length < 5) {
      ...
      const relatedPlaces = availablePlaces.filter(place => 
        placeTypesToUse.includes(place.placeType) && place.placeType !== currentLocation.type
      );
    ...
    candidatePlaces = candidatePlaces.filter((place, index, self) => 
      index === self.findIndex(p => p.id === place.id)
    );
    ...
    const randomIndex = Math.floor(Math.random() * Math.min(5, topRatedPlaces.length)); // Random from top 5
    newLocation = topRatedPlaces[randomIndex];
    
    console.log(`🎲 Selected from ${candidatePlaces.length} candidates (${sameTypePlaces.length} same type, ${topRatedPlaces.length} top-rated)`);
    console.log(`🎯 Picked: ${newLocation.displayName} (${newLocation.placeType}, rating: ${newLocation.rating})`);
